...
if not defined last_qu_kan_guang_gao_time (
	rem echo "go utils_gettimestamp_hmsp"
	call :get_last_qu_kan_guang_gao_time
)  

set "now_qu_kan_guang_gao_time="
rem echo "go utils_gettimestamp_hmsp"
call utils_gettimestamp_hms.bat now_qu_kan_guang_gao_time
rem echo now_qu_kan_guang_gao_time:%now_qu_kan_guang_gao_time%

set /a "sec_count=%now_qu_kan_guang_gao_time%-%last_qu_kan_guang_gao_time%"
echo 距离上次去看广告时间：%sec_count%

if %sec_count% gtr %qukanguanggaorenwujiange% (
	rem echo 可以去看广告
...
call utils_gettimestamp_hms.bat last_qu_kan_guang_gao_time
...
echo 滑动到广告按钮
adb -s %device_name% shell input swipe 10 %swipe_to_guanggao_y% 10 0 
adb -s %device_name% shell input swipe 10 %swipe_to_guanggao_y% 10 0 
adb -s %device_name% shell input swipe 10 %swipe_to_guanggao_y% 10 0

 
echo 点击广告
adb -s %device_name% shell input tap %tap_go_guang_gao_x% %tap_go_guang_gao_y%
